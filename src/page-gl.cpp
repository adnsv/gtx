...
private:
    ...
    auto new_page(const texture::texel_size& sz, bool wrap)
...
auto texture::page::update(
    const texture::texel_box& box, const uint32_t* data, size_t data_stride) -> bool
{
...
void texture::page::setup(texture::texel_size const& sz, bool wrap)
{
    ...
    pd_ = gtx::factory::get()->new_page(sz, wrap);
}
...
auto factory::new_page(const texture::texel_size& sz, bool wrap)
    -> std::shared_ptr<texture::page_data>
{
    if (!sz.w || !sz.h)
        return {};
    ...
    auto pg = std::shared_ptr<texture::page_data>(
        new texture::page_data{gln, sz, wrap});
    pages.push_back(pg);
    return pg;
}
