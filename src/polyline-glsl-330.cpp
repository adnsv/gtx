...
polyline::polyline()
    : vertex_shader{GL_VERTEX_SHADER, R"(
        #version 330
        in vec2 in_pos;
        in float in_thk;
        in vec4 in_clr;
    ...
    , geometry_shader{GL_GEOMETRY_SHADER, R"(
        #version 330
        ...
        {	
            ...
            if (p0 == p1) {
                ...
                t1a = mix(t1a, m1, float(cw1 < 0));
                t1b = mix(t1b, m1, float(cw1 >= 0));
            }
            ...
            if (p2==p3) {
                ...
                t2a = mix(t2a, m2, float(cw2 < 0 || dp2 > 0.85));
                t2b = mix(t2b, m2, float(cw2 >= 0 || dp2 > 0.85));
            }
            emit(p1 + t1a * th1, c1);
            emit(p1 - t1b * th1, c1);
            emit(p2 + t2a * th2, c2);
    ...
    , fragment_shader{GL_FRAGMENT_SHADER, R"(
        #version 330
